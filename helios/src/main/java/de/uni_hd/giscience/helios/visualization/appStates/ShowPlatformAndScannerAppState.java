	...
	public void update(float tpf) {
		...
		if (sim.getScanner() != mCurrentScanner) {
			...
			if (mCurrentScanner != null) {
				...
				/*for(int i=0 ;i<mCurrentScanner.beams.length;i++){
				emitterNode.setLocalTranslation(am2jme_vector(mCurrentScanner.beams[i]));
				}*/
		...
		if (mCurrentScanner != null) {
			...
			//for(int i=0;i<mCurrentScanner.beamDeflector.length;i++){
