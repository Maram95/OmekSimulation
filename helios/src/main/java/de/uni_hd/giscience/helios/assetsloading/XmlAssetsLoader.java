...
System.out.println("@@@"+type +"@@@@");
	...
	public PlatformSettings createPlatformSettingsFromXml(Element node) {
		...
		//settings.error=(Double) getAttribute(node,"Error",Double.class ,template.error);
		return settings;
	}

	public Velodyne createScannerFromXml(Element scannerNode) {
		...
		Velodyne scanner = new Velodyne(beamDiv_rad, emitterPosition, emitterAttitude, pulseFreqs, pulseLength_ns, visModel);
		...
		// here we can change the rotating.
		...
		//for(int i=0;i<scanner.detector.length;i++){
